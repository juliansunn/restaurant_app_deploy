    ...
    if "user_id" in request.session:
        ...
        rest_list = (
            Restaurant.objects.filter(
                location_id__in=Location.objects.filter(
                    city__in=Location.objects.filter(
                        id__in=user.favorites.all().values("location_id").distinct()
                    )
                    .values("city")
                    .distinct(),
                    state__in=Location.objects.filter(
                        id__in=user.favorites.all().values("location_id").distinct()
                    )
                    .values("state")
                    .distinct(),
            ...
            .exclude(favorites=user)
            .order_by("-rating", "review_count", "-location__city", "price")
        )
...
def category_explore(request):
    ...
    if user:
        if request.method == "GET":
            ...
            context["user"] = user[0]
            context["states"] = (
                Location.objects.all().values_list("state", flat=True).distinct()
            )
            if city and state and category:
                if category.strip() == "all":
                    restaurants = Restaurant.objects.filter(
                        location_id__in=Location.objects.filter(city=city)
                    ).order_by("rating")
                else:
                    restaurants = Restaurant.objects.filter(
                        location_id__in=Location.objects.filter(city=city),
                        categories__in=Category.objects.filter(title=category),
                    ).order_by("name")
...
def related_restaurants(restaurant):
    ...
    if categories:
        rel_rests = Restaurant.objects.filter(
            location_id__in=(
                Location.objects.filter(
                    city=restaurant.location.city, state=restaurant.location.state
                )
            ),
            categories__in=restaurant.categories.all(),
        )
    ...
    if "user_id" in request.session:
        ...
        if user:
            context = {
                ...
                "user_prof_reviews": Review.objects.filter(
                    creator=user_prof[0]
                ).order_by("-updated_at"),
            }
    ...
    if data:
        ...
        for test_rest in restaurants:
            ...
            if test_rest["image_url"] == "":
                test_rest[
                    "image_url"
                ] = "https://s3-media1.fl.yelpcdn.com/bphoto/UaybPI6fcFGFsvftdW54JA/o.jpg"
            is_in_db = Restaurant.objects.filter(
                name=test_rest["name"], url=test_rest["url"]
            )
            if is_in_db:
                ...
                new_restaurant = Restaurant.objects.create(
                    ...
                    name=test_rest["name"],
                    alias=test_rest["alias"],
                    is_closed=test_rest["is_closed"],
                    url=test_rest["url"],
                    review_count=test_rest["review_count"],
                    rating=test_rest["rating"],
                    latitude=test_rest["coordinates"]["latitude"],
                    longitude=test_rest["coordinates"]["longitude"],
                    phone_number=test_rest["display_phone"],
                    price=test_rest["price"],
                )
                ...
                for c in category_types:
                    ...
                    if not c_type:
                        new_c_type = Category.objects.create(
                            title=c["title"], alias=c["alias"]
                        )
                    else:
                        new_c_type = c_type[0]
...
def state_change(request):
    if request.method == "GET":
        data = {
            "cities": list(
                Location.objects.filter(state=request.GET["state"])
...
def city_change(request):
    categories = (
        Category.objects.filter(
            Category__in=(
                Restaurant.objects.filter(
                    location_id__in=Location.objects.filter(city=request.GET["city"])
                )
            )
        )
        .distinct()
...
def new_loc_autocomplete(request):
    if request.is_ajax():
        ...
        search_qs = (
            Location.objects.filter(Q(city__contains=q)).values_list("city").distinct()
        )
        results = []
        if search_qs:
...
def edit_profile(request):
    ...
    return redirect(f"/dashboard/user/show/{user.pk}")
